...
// House Special -- the comments API. Shared by materials and blends, addressed by
// target_type + target_id, with threaded replies.
...
function normalizeComment(value: unknown): HallComment | null {
  ...
  return {
    ...
    authorId,
    authorName: cleanText(record.author_name ?? record.authorName, 80) || "Anonymous guest",
    content,
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ ok: false, error: formatSupabaseError(err), comments: [] }, { status: getMixologySupabaseConfig() ? 500 : 503 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!targetResult.data[0]) return NextResponse.json({ ok: false, error: "That content does not exist, or has been taken down." }, { status: 404 });
    if (parentId) {
      ...
      if (!parentResult.data[0]) return NextResponse.json({ ok: false, error: "The comment being replied to does not exist, or has been deleted." }, { status: 404 });
    }
    ...
    if (!comment) return NextResponse.json({ ok: false, error: "Could not save the comment." }, { status: 500 });
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!target || !type || !targetId || !commentAuthorId) {
      return NextResponse.json({ ok: false, error: "That comment does not exist, or has been deleted." }, { status: 404 });
    }
    ...
    if (commentAuthorId !== account.id && ownerAuthorId !== account.id) {
      return NextResponse.json({ ok: false, error: "You do not have permission to delete this comment." }, { status: 403 });
    }
    ...
    if (deleteIds.length === 0) return NextResponse.json({ ok: false, error: "That comment does not exist, or has been deleted." }, { status: 404 });
