...
const SEND_FILE_USAGE_GUIDE = [
    ...
    "示例：",
    '[CallTool:发送文件({"url":"https://example.com/song.mp3","type":"audio","title":"为你写的歌"})]',
].join("\n");
...
const LOCAL_DATA_LIBRARY_USAGE_GUIDE = [
    ...
    "  - fields/select (string[]): 可选，只返回指定字段；支持点路径，例如 mediaData.label",
    "示例：",
    '[CallTool:搜索资料记录({"path":"/chat","query":"沈既川","limit":30,"fields":["id","role","content","createdAt"]})]',
    "",
    "动作：读取资料记录",
    "参数：",
...
const TOOLBOX_MANAGEMENT_USAGE_GUIDE = [
    ...
    "示例：",
    '[CallTool:添加REST套件({"name":"网页资料工具","description":"搜索、读取和整理网页内容","enabled":true})]',
    "",
    "动作：更新REST套件",
    "参数：",
    "  - id/name: 要更新的 AI 套件",
    ...
    "示例：",
    '[CallTool:更新REST套件({"name":"网页资料工具","updates":{"description":"网页搜索、正文读取和内容整理"}})]',
    "",
    "动作：设置REST套件启用",
    "参数：",
    ...
    "示例：",
    '[CallTool:设置REST套件启用({"name":"网页资料工具","enabled":true})]',
    "",
    "动作：删除REST套件",
    "参数：",
    "  - id/name: 要删除的 AI 套件",
    "示例：",
    '[CallTool:删除REST套件({"name":"网页资料工具"})]',
    "",
    "动作：添加REST工具",
    "参数：",
    ...
    "  - enabled (boolean): 是否启用，默认 true",
    "示例：",
    '[CallTool:添加REST工具({"packageName":"网页资料工具","name":"读取网页正文","description":"读取网页 URL 并返回正文","endpoint":"https://r.jina.ai/http://{{{url}}}","method":"GET","directFetch":false,"parameterSchema":"{\\"type\\":\\"object\\",\\"properties\\":{\\"url\\":{\\"type\\":\\"string\\",\\"description\\":\\"网页 URL，建议不带 https:// 或 http://\\"}},\\"required\\":[\\"url\\"]}"})]',
    "",
    "动作：更新REST工具",
    "参数：",
    "  - id/name: 要更新的 AI 工具",
    "  - updates (object, 必填): 要更新的字段",
    "示例：",
    '[CallTool:更新REST工具({"name":"读取网页正文","updates":{"endpoint":"https://api.example.com/read","bodyTemplate":"{\\"input\\":\\"{{url}}\\"}"}})]',
    "",
    "动作：设置REST工具启用",
    "参数：",
    ...
    "  - enabled (boolean, 必填): true 启用，false 停用",
    "示例：",
    '[CallTool:设置REST工具启用({"name":"读取网页正文","enabled":true})]',
    "",
    "动作：删除REST工具",
    "参数：",
    "  - id/name: 要删除的 AI 工具",
    "示例：",
    '[CallTool:删除REST工具({"name":"读取网页正文"})]',
    "",
    "动作：添加组合工具套件",
    "参数：",
    ...
    "  - enabled (boolean): 是否启用，默认 true",
    "示例：",
    '[CallTool:添加组合工具套件({"name":"网页研究流程","description":"搜索、读取、整理和记录网页资料","enabled":true})]',
    "",
    "动作：添加组合工具",
    "参数：",
    ...
    "  - enabled (boolean): 是否启用，默认 true",
    "示例：",
    '[CallTool:添加组合工具({"packageName":"网页研究流程","name":"搜索并整理网页","description":"搜索关键词并把搜索结果整理为可继续使用的摘要","parameterSchema":"{\\"type\\":\\"object\\",\\"properties\\":{\\"query\\":{\\"type\\":\\"string\\",\\"description\\":\\"搜索关键词\\"}},\\"required\\":[\\"query\\"]}","steps":[{"toolName":"搜索","toolType":"rest","argsTemplate":{"query":"{{input.query}}"},"saveAs":"search"}],"outputTemplate":"{{steps.search.data}}","enabled":true})]',
    "脚本步骤示例：",
