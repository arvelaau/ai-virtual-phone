...
export type NeteaseComment = {
    ...
    likedCount: number;
    time: number;
    /** e.g. "Guangdong" — Netease shows the commenter's region */
    ipLocation?: string;
    /** number of floor replies under this comment */
    replyCount?: number;
};
...
/** Get a playable URL for a Netease song, with a concrete failure reason. */
export async function getNeteasePlayInfo(songId: number): Promise<NeteasePlayInfo> {
    const base = neteaseBase();
    if (!base) return { url: null, trial: false, reason: "Music API is not configured" };
    try {
        ...
        if (fee === 1) {
            return { url: null, trial: false, reason: loggedIn ? "VIP track, and this account has no membership" : "VIP track — sign in to your NetEase account in settings first" };
        }
        if (fee === 4) {
            return { url: null, trial: false, reason: "Track from a paid album; it must be bought before it will play" };
        }
        // Ask check/music for a human-readable reason (e.g. no licence in this region).
        // NOTE: the reason comes back from NetEase in Chinese and is passed through as-is —
        // it is provider text, not our UI copy. `亲爱的` ("dear ...") is the polite prefix
        // their API prepends; stripping it is a match against their wording, so it must
        // stay Chinese, same category as the provider-error keyword lists elsewhere.
        try {
        ...
        return { url: null, trial: false, reason: "This track cannot be played at the moment" };
    } catch (e) {
        console.warn("[MusicService] Get play URL failed:", e);
        return { url: null, trial: false, reason: "Network problem, could not load" };
    }
}

/** Get a playable URL for a Netease song */
...
function mapComment(c: any): NeteaseComment {
    return {
        id: c.commentId,
        userName: c.user?.nickname || "NetEase user",
...
/** Post a comment on a song or playlist (requires a Netease login cookie) */
export async function postSongComment(songId: number, content: string, resType: NeteaseCommentResType = 0): Promise<{ ok: boolean; message: string }> {
    const base = neteaseBase();
    if (!base) return { ok: false, message: "Music API is not configured" };
    if (!loadNeteaseCookie()) return { ok: false, message: "Sign in to your NetEase account first to post a comment" };
    try {
        ...
        const data = await resp.json();
        if (data?.code === 200) return { ok: true, message: "Comment posted" };
        return { ok: false, message: data?.message || data?.msg || "Could not post the comment" };
    } catch (e) {
        return { ok: false, message: e instanceof Error ? e.message : "Could not post the comment" };
    }
}

/** Subscribe to or unsubscribe from a playlist (requires a Netease login cookie) */
export async function subscribePlaylist(playlistId: number, subscribe: boolean): Promise<{ ok: boolean; message: string }> {
    const base = neteaseBase();
    if (!base) return { ok: false, message: "Music API is not configured" };
    if (!loadNeteaseCookie()) return { ok: false, message: "Sign in to your NetEase account first to save a playlist" };
    try {
        const resp = await fetch(withNeteaseParams(`${base}/playlist/subscribe?t=${subscribe ? 1 : 2}&id=${playlistId}&timestamp=${Date.now()}`));
        const data = await resp.json();
        if (data?.code === 200) return { ok: true, message: subscribe ? "Playlist saved" : "Removed from saved" };
        if (data?.code === 501) return { ok: true, message: "This playlist is already saved" };
        return { ok: false, message: data?.message || data?.msg || "That did not work" };
    } catch (e) {
