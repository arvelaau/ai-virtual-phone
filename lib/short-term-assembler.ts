...
import { loadReviewProjectionEntries } from "./review-memory";
...
export type NativeTimelineEntry = {
    id: string;
    sourceApp: "chat" | "moments" | "story" | "vn" | "map" | "game" | "diary" | "xiaohongshu" | "interview_magazine" | "cocreate" | "checkphone" | "custom_app" | "review";
    sourceDetail?: "direct" | "group" | "system" | "story" | "chat_offline" | "chat_offline_session" | "game" | "diary_entry" | "notewall" | "xiaohongshu" | "black_market_theater" | "interview_issue" | "interview_shared_issue" | "cocreate_project" | "checkphone" | "custom_app_event" | "review_journal" | "review_published"; // chat sub-type: 1:1 vs group chat vs system note
...
export function loadNativeTimeline(
    ...
    // ── Review app projections ──
    const reviewEntries = loadReviewProjectionEntries(characterId, {
        afterTimestamp: options?.afterTimestamp,
    });
    for (const reviewEntry of reviewEntries) {
        entries.push({
            id: reviewEntry.id,
            sourceApp: "review",
            sourceDetail: reviewEntry.authorType === "character" ? "review_published" : "review_journal",
            authorType: reviewEntry.authorType,
            timestamp: reviewEntry.timestamp,
            content: formatStoredPromptEventContent(reviewEntry.content, {
                label: "Review",
                timestamp: reviewEntry.timestamp,
                timeAware,
                timestampOptions,
...
const FEATURE_ORDER: Record<string, number> = { map: 0, game: 0.5, moments: 1, xiaohongshu: 1.5, checkphone: 1.7, story: 2, vn: 2, theater: 2.2, interview: 2.35, cocreate: 2.4, diary_entry: 2.45, notewall: 2.5, review: 2.55, custom_app: 2.6, group_chat: 3, chat: 4 };
...
export function prepareShortTermContext(
    ...
    const reviewEntries = timeline.filter(e => e.sourceApp === "review");
    if (reviewEntries.length > 0) {
        raw.push({ tag: "recent_review", order: FEATURE_ORDER.review, entries: reviewEntries });
    }
                                                        ...
                                                        entry.sourceApp === "story" && entry.sourceDetail === "black_market_theater" ? "recent_theater" :
                                                            entry.sourceApp === "diary" && entry.sourceDetail === "diary_entry" ? "recent_diary" :
                                                                entry.sourceApp === "diary" && entry.sourceDetail === "notewall" ? "recent_notewall" :
                                                                    entry.sourceApp === "review" ? "recent_review" :
