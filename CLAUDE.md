...
**Smoke test: PASSED (2026-08-18, user-confirmed).** Removed a friend; they stayed removed, and the chat history was not deleted. That covers the half of this fix a fixture cannot prove on its own — that the session survives a removal in the real app, which is the entire reason the rescue path exists.
...
**Smoke test: PASSED (2026-08-19, user-confirmed).** The four local tabs behave as expected. The `hall` tab reports "not open yet" (`missing_supabase_env` → 503, same as Game Hall) — that is the designed state without Supabase, not a fault.
...
#### Deferred, and why
The **prompt-structure commits** — **PORTED (`3ac3afb`)**. Sections moved `##` → `#`, and every input box became its own `##` block titled with the label the editor shows on it, so what an author writes matches what the model receives and `###` down is free for their own structure. The preamble's first sentence now names the character; a stacked slot gives each material a `##` titled with its name; the cognition box's label is literally `Initial awareness of {{user}}` in BOTH editor and assembler, macro included.

**Fixture group F is why this was safe to attempt, and it worked — but with a lesson.** It failed the moment the headings moved. However the suite went **58 → 48 while showing only four failures**: the row loop filters on the heading prefix, so ten assertions SILENTLY DISAPPEARED rather than failing. **A filtered loop needs a count guard** — added as F1b. Same defect class as a vacuous assertion, but harder to spot, because the total shrinking looks like the suite got smaller rather than weaker.
