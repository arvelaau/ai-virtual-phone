    ...
    return (
        <div className="relative flex-1 h-full">
            <PageShell
                leftAction={
                    <div className="flex items-center min-w-max">
                        <button className="page-back-btn shrink-0 mr-2" type="button" onClick={onCloseApp} aria-label="Back">
                        ...
                        <div className="flex items-center gap-[10px]">
                            ...
                            <div className="flex flex-col whitespace-nowrap">
                                <span className="ts-16 font-bold text-[var(--c-text-title)] leading-tight">{identity?.name || "User"}</span>
                                <div className="flex items-center gap-1 mt-1">
                                    <span className="w-[8px] h-[8px] rounded-full bg-[#2dd36f]"></span>
                                    <span className="ts-10 text-[var(--c-icon)] font-medium">Online</span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="g-dropdown absolute top-[40px] right-0 py-2 px-0 w-[140px] z-[100]">
                                ...
                                <MenuOption
                                    ...
                                    onClick={() => {
                                        ...
                                        setGreetingText(identity?.name ? `I'm ${identity.name}` : "Hello");
                                    }}
                                />
                            </div>
                        )}
                ...
                <div className="px-5 pt-2 flex flex-col">
                    {(() => {
                            ...
                            const showMascot = mascotSettings.chatEnabled
                                && listTab !== "group"
                                && (!keyword || (mascotSettings.nickname || "AI Assistant").toLowerCase().includes(keyword));
                            ...
                            .filter(s => {
                                ...
                                if (s.isGroup) return (s.groupName || "Group Chat").toLowerCase().includes(keyword);
                            ...
                            if (!showMascot && regularItems.length === 0) {
                                return (
                                    <div className="px-5 py-10 text-center text-[var(--c-icon)] ts-14">
                                        No chats yet. Tap "+" in the top right to start a chat
                                    </div>
                                );
                            }
                            return (
                                <>
                                    {showMascot && (
                                        <MascotSessionItem
                                            name={mascotSettings.nickname || "AI Assistant"}
            ...
            {isSearchModalOpen && (
                ...
                <PageShell title="Add Friend" onBack={() => setIsSearchModalOpen(false)}>
                                ...
                                {searchQuery.trim().length > 0 && (
                                    ...
                                    >
                                        ...
                                        <div className="menu-label-group">
                                            <span className="menu-label">Search: <span className="text-[var(--c-success)]">{searchQuery}</span></span>
                                        </div>
                                    </button>
                                )}
                            </div>
                                ...
                                return (
                                    <div className="menu-group" style={{ marginTop: 12 }}>
                                        <div className="menu-item" style={{ pointerEvents: "none" }}>
                                            <span className="menu-desc">Characters who aren't friends yet (tap to fill in ID)</span>
                                        </div>
                                        {!mascotSettings.chatEnabled && (
                                            ...
                                            >
                                                ...
                                                <div className="menu-label-group" style={{ minWidth: 0 }}>
                                                    <span className="menu-label">AI Assistant</span>
                                                    <span className="menu-desc">Built-in creative assistant</span>
                                                </div>
                                            </button>
                                        )}
                                        {candidates.map(c => (
                    ...
                    {searchResult === null && (
                        <div className="ui-empty">
                            <span className="menu-desc">This user doesn't exist</span>
                        </div>
                    )}
                            ...
                            <div className="menu-group">
                                <div className="menu-item !items-start">
                                    ...
                                    <div className="menu-label-group">
                                        ...
                                        <div className="menu-desc">WeChat ID: {searchResult.wechatID || "N/A"}</div>
                                        <div className="menu-desc">Bio: {searchResult.persona ? searchResult.persona.slice(0, 30) + (searchResult.persona.length > 30 ? "..." : "") : "None"}</div>
                                    </div>
                                </div>
                            </div>
                            <button
                    ...
                    {isSendingRequest && searchResult && (
                        <div className="page-menu">
                            <p className="menu-group-desc mx-0">Send Friend Request</p>
                            <div className="menu-group">
                                <div className="menu-item !items-start">
                                    <textarea
                                        autoFocus
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-dialog" onClick={e => e.stopPropagation()}>
                <span className="modal-header-title">Select Contact</span>
                {enrichedContacts.length === 0 ? (
                    <span className="menu-desc">No contacts yet. Add a friend first.</span>
                ) : (
...
function SessionItem({ session, onSelect, isPinned }: { session: ChatSession, onSelect: () => void, isPinned?: boolean }) {
    ...
    const groupAvatarItems = isGroup
        ? [
            ...(userIdentity ? [{ id: "self", name: userIdentity.name || "Me", avatar: userIdentity.avatarUrl || "" }] : []),
            ...
            <div className="flex-1 overflow-hidden h-[48px] flex flex-col justify-center gap-1">
                <div className="flex justify-between items-center">
                    <span className="ts-16 font-medium text-[var(--c-text-title)] truncate">
                        {isGroup ? (session.groupName || "Group Chat") : (session.alias || character?.name || `User_${session.contactId.slice(-4)}`)}
                    </span>
