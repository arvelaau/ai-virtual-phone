...
export function VideoCallScreen({ session, character, onEnd, onConnect, initiator = "user" }: VideoCallScreenProps) {
    ...
    const stateLabel = (): string => {
        switch (callState) {
            ...
            case "IDLE": return isMuted ? "Muted" : "Video Call";
    ...
    const runConversationTurn = useCallback(async (userText?: string) => {
        ...
        try {
            ...
            if (stateRef.current !== "ENDED") {
                setSubtitles(prev => [...prev, { id: `err-${Date.now()}`, role: "assistant", text: `⚠️ ${error?.message || "Failed to send"}` }]);
                setCallState("IDLE");
            }
        }
                ...
                {callState !== "ENDED" && callState !== "CONNECTING" ? androidTextInputOnly ? (
                    ...
                    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 26, width: "100%" }}>
                        ...
                        <button
                            ...
                            title={cameraEnabled ? "Switch camera" : "Turn on camera first"}
                        >
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                <path d="M23 4v6h-6" />
                                <path d="M1 20v-6h6" />
                    ...
                    <>
                        ...
                        <button
                            ...
                            title={isMuted ? "Microphone muted" : "Microphone on"}
                        >
                            {isMuted ? (
                                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <line x1="1" y1="1" x2="23" y2="23" />
                    ...
                    <div className="ts-14 opacity-70">Call Ended</div>
                )}
            </div>
