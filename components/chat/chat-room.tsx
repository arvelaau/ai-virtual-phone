...
function MemoryWriteRequestCard({
    ...
    const statusText = status === "approved" ? "Saved to long-term memory" : status === "ignored" ? "Ignored this time" : "Waiting for your confirmation";
    ...
    return (
        <div className="w-[280px] rounded-2xl border border-[var(--c-border)] bg-[var(--c-card)]/95 backdrop-blur px-4 py-3 flex flex-col gap-3 ui-bubble-shadow">
            <div className="flex items-center justify-between gap-3">
                <span className="menu-label">They want to remember this</span>
            ...
            {(reason || typeof importance === "number") && (
                <div className="flex flex-col gap-1">
                    {reason && <span className="menu-desc !mt-0">Reason: {reason}</span>}
                    {typeof importance === "number" && <span className="menu-desc !mt-0">Importance: {importance.toFixed(2)}</span>}
                </div>
            )}
            {status === "pending" ? (
                <div className="flex gap-2">
                    <button onClick={() => void onApprove(msg)} className="ui-btn ui-btn-primary flex-1">Confirm</button>
                    <button onClick={() => onIgnore(msg)} className="ui-btn ui-btn-outline flex-1">Ignore</button>
                </div>
            ) : null}
        </div>
    );
    ...
    return (
        <>
            <div className="chat-system-instruction-head">
                <span className="chat-system-instruction-title">System Instruction</span>
            </div>
...
const ChatTextInputBar = memo(forwardRef<ChatTextInputHandle, {
    ...
    const plusMenuItems = [
        { icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--c-text)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><circle cx="8.5" cy="8.5" r="1.5" /><polyline points="21 15 16 10 5 21" /></svg>, label: "Photo Wall", onClick: () => onOpenRichModal("photo") },
        { icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--c-text)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><line x1="7" y1="8" x2="17" y2="8" /><line x1="7" y1="12" x2="14" y2="12" /><line x1="7" y1="16" x2="11" y2="16" /></svg>, label: "Text Image", onClick: () => onOpenRichModal("text_photo") },
        { icon: <AlertCircle size={22} strokeWidth={1.5} color="var(--c-text)" />, label: "System Instruction", onClick: () => onOpenRichModal("system_instruction") },
        { icon: <Clapperboard size={22} strokeWidth={1.5} color={theaterMode ? "var(--c-icon-active)" : "var(--c-text)"} />, label: "Side-story Mode", active: theaterMode, onClick: onToggleTheaterMode },
        { icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--c-text)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M23 7l-7 5 7 5V7z" /><rect x="1" y="5" width="15" height="14" rx="2" ry="2" /></svg>, label: "Video Call", onClick: onStartVideoCall },
        { icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--c-text)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3Z" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="22" /></svg>, label: "Voice Call", onClick: onStartVoiceCall },
        { icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--c-text)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2" /><line x1="2" y1="10" x2="22" y2="10" /></svg>, label: "Red Envelope", onClick: () => onOpenRichModal("red_packet") },
        { icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--c-text)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><text x="12" y="16" textAnchor="middle" fontSize="12" fill="var(--c-text)" stroke="none">¥</text></svg>, label: "Transfer", onClick: () => onOpenRichModal(isGroup ? "transfer_target" : "transfer") },
        { icon: <Gift size={22} strokeWidth={1.5} color="var(--c-text)" />, label: "Gift", onClick: () => onOpenRichModal("gift") },
        { icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--c-text)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>, label: "Location", onClick: () => onOpenRichModal("location") },
        { icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--c-text)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3Z" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="22" /><line x1="8" y1="22" x2="16" y2="22" /></svg>, label: "Voice Note", onClick: () => onOpenRichModal("voice_msg") },
        ...
        <div className="chat-input-bar chat-room-main-pane flex flex-col" data-ui="input">
            {theaterMode && (
                <div className="chat-theater-mode-strip" role="status">
                    ...
                    <span className="chat-theater-mode-title">Side-story Mode</span>
                    <button
                        type="button"
                        className="chat-theater-mode-close"
                        onClick={onCloseTheaterMode}
                        aria-label="Close side-story mode"
                        title="Close side-story mode"
                    >
            ...
            {quotingMessage && (
                <div className="chat-quote-bar">
                    <div className="flex-1 ts-12 text-[var(--c-icon)] overflow-hidden text-ellipsis whitespace-nowrap">
                        Quoting {quotingMessage.role === "user" ? "you" : characterName}: {quotingMessage.content.slice(0, 40)}
                    </div>
            ...
            <textarea
                ...
                placeholder={inputLocked
                    ? (isSpectator ? "Spectating, you're not in this group" : `Muted, ${Math.ceil(muteRemainingMs / 60000)} min remaining`)
                    : (theaterMode ? "Write a side-story command..." : undefined)}
            />
    ...
    return (
        <div className="chat-input-bar chat-room-main-pane flex flex-col" data-ui="input">
            <textarea
                ...
                placeholder={isSpectator ? "Spectating, tap the button on the right to advance their offline interaction" : undefined}
            />
            <div className="chat-input-actions">
                <button
                    type="button"
                    ...
                    className="ui-bare-btn text-[var(--c-text)]"
                    aria-label="Return to online mode"
                ...
                <button
                    ...
                    className="ui-bare-btn text-[var(--c-text)]"
                    aria-label={isOfflineGenerating ? "Stop offline generation" : "Send"}
                    title={isOfflineGenerating ? "Stop offline generation" : "Send"}
                >
                    {isOfflineGenerating ? (
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                            <circle cx="12" cy="12" r="10" />
...
export function ChatRoom({ session, onBack }: ChatRoomProps) {
    ...
    const handleTranslateReasoning = async () => {
        ...
        try {
            ...
            else setReasoningTranslateError(result.error || "Translation failed, please try again");
        } catch {
            setReasoningTranslateError("Translation failed, please try again");
        } finally {
            setReasoningTranslating(false);
        }
    };
    ...
    const handleAIMediaAction = (actionType: string, charN: string, userN: string) => {
        ...
        const refundReason = actionType === "decline_red_packet" ? "Red envelope refund" : actionType === "decline_transfer" ? "Transfer refund" : null;
    ...
    const handleGroupTransferAction = (action: "accept" | "decline", claimerName: string, ownerName?: string) => {
        ...
        const refundData = action === "decline" && targetMsg.role === "user"
            ? refundOutgoingMoneyMessage(targetMsg, "Transfer refund")
            : targetMsg.mediaData;
            ...
            for (const part of parts) {
                ...
                if (part.mediaType === "poke") {
                    ...
                    dispatchChatMessageNotice({
                        sessionId: session.id,
                        senderName: session.groupName || "Group Chat",
                ...
                const body = msg.content.trim()
                    || (msg.mediaType === "media_file" && msg.mediaData?.fileType === "image" && msg.mediaData?.label
                        ? `Sent a photo: ${msg.mediaData.label}`
                        : (msg.mediaType ? "Sent a message" : ""));
                dispatchChatMessageNotice({
                    sessionId: session.id,
                    senderName: session.groupName || "Group Chat",
    ...
    const clearStuckGeneration = () => {
        ...
        syncMessagesFromStorage();
        showChatToast("Generation stopped");
    };
    ...
    const handleMusicCardPlay = async (title: string, artist?: string) => {
        const musicBridge = getMusicControlBridge();
        if (!musicBridge) { showChatToast("Music player not ready"); return; }
        showPersistentChatToast("Loading music...");
        try {
            const found = await findPlayableMatch(title, artist);
            if (!found) {
                showChatToast("Couldn't find that song~");
                return;
            }
            const { result: match, playUrl } = found;
            if (match.source === "local" && match.localTrack) {
    ...
    const splitAndSaveAIMessages = async (
        ...
        const mediaLabels: Record<string, string> = {
            red_packet: "Sent a red envelope",
            transfer: "Sent a transfer",
            payment_request: "Sent a payment request",
            sticker: "Sent a sticker",
            image: "Sent a photo",
            location: "Shared a location",
            audio: "Sent a voice message",
            music_share: "Shared music",
            xiaohongshu_note_share: "Shared a Xiaohongshu post",
            app_card: "Shared an app card",
            quote: "Quoted reply",
        };
        const getNoticeBody = (m: ChatMessage): string => {
            ...
            if (m.mediaType === "sticker") return `Sent a sticker ${m.mediaData?.label || ""}`.trim();
            if (m.mediaType === "image") return m.mediaData?.label ? `Sent a photo: ${m.mediaData.label}` : "Sent a photo";
            if (m.mediaType === "media_file" && m.mediaData?.fileType === "image") {
                return m.mediaData?.label ? `Sent a photo: ${m.mediaData.label}` : "Sent a photo";
            }
            if (m.mediaType === "location") return `Shared a location: ${m.mediaData?.label || ""}`.trim();
            if (m.mediaType === "audio") return `Sent a voice message: ${m.mediaData?.label || ""}`.trim();
            if (m.mediaType === "music_share") return `Shared music: ${m.mediaData?.musicTitle || ""}`.trim();
            if (m.mediaType === "xiaohongshu_note_share") return `Shared a Xiaohongshu post: ${m.mediaData?.xiaohongshuTitle || ""}`.trim();
            if (m.mediaType === "app_card") return `Shared ${m.mediaData?.appName || "an app"} card: ${m.mediaData?.appCardTitle || m.mediaData?.appCardSummary || ""}`.trim();
            if (m.mediaType === "quote") return `Quoted reply: ${m.mediaData?.quotePreview || ""}`.trim();
            if (m.mediaType === "payment_request") return `Sent a payment request: ${m.mediaData?.paymentRequestAmountLabel || m.mediaData?.amount || ""}`.trim();
    ...
    const renderGroupRoleBadge = (senderCharacterId?: string) => {
        ...
        if (role === "owner") return <span className="chat-role-badge chat-role-badge-owner">Owner</span>;
    ...
    const debitOutgoingMoneyMessage = (
        ...
        if (amount <= 0) {
            showChatToast("Invalid amount");
            return { ok: false };
        }
        const isRedPacket = mediaType === "red_packet";
        const result = payWithWalletBalance({
            amount,
            title: isRedPacket ? "Send Red Envelope" : "Send Transfer",
            detail: `${session.isGroup ? session.groupName || "Group Chat" : character?.name || "Chat"}: ${isRedPacket ? "Send Red Envelope" : "Send Transfer"} ${amount.toFixed(2)} yuan`,
            category: isRedPacket ? "Red Envelope" : "Transfer",
        });
        if (!result.ok || !result.transaction) {
            showChatToast(result.error ?? "Insufficient balance");
            return { ok: false };
        }
        return {
            ok: true,
    ...
    const refundOutgoingMoneyMessage = (msg: ChatMessage, reason: "Red envelope refund" | "Transfer refund"): ChatMessage["mediaData"] => {
        ...
        const result = creditWalletBalance(amount, reason, `${reason}: ${data.label || msg.content || "Chat payment"}`, "Chat refund");
    ...
    const creditIncomingMoneyMessage = (msg: ChatMessage, actionType: string): ChatMessage => {
        ...
        const result = creditWalletBalance(
            safeAmount,
            actionType === "accept_red_packet" ? "Claimed Red Envelope" : "Received Payment",
            `${actionType === "accept_red_packet" ? "Claimed Red Envelope" : "Received Payment"}: ${data?.label || msg.content || "Chat payment"}`,
            actionType === "accept_red_packet" ? "Red Envelope" : "Transfer",
        );
    ...
    const handleOpenCustomPlusAction = useCallback((action: RegisteredCustomAppChatPlusAction) => {
        ...
        if (!app) {
            showChatToast("This custom app no longer exists");
    ...
    const sendShoppingGiftMessage = (gift: ShoppingGiftCandidate, recipient?: Character): boolean => {
        if (session.isGroup && !recipient) {
            showChatToast("Please select a gift recipient");
            return false;
        }
        ...
        if (sent) showChatToast("Gift sent");
        return sent;
    };
    ...
    const triggerAIResponse = async () => {
        ...
        try {
            ...
            const errorMsg = pushChatMessage({
                sessionId: session.id,
                role: "system",
                content: `⚠️ Send failed: ${error?.message || String(error)}`
            });
            setMessages(prev => [...prev, errorMsg]);
        } finally {
            if (finishGenerationRun(session.id, generationRunId)) {
    ...
    const ensureGroupSpeakPermission = (): boolean => {
        ...
        if (session.isSpectator) {
            showChatToast("Spectator groups can't send messages, only tap Generate");
            return false;
        }
        const muteMs = getGroupMuteRemainingMs(session, GROUP_SELF_KEY);
        if (muteMs > 0) {
            showChatToast(`You're muted, ${formatMuteRemainingLabel(muteMs)} remaining`);
            return false;
        }
        return true;
    };
    ...
    const handleOfflineSend = (inputText: string): boolean => {
        ...
        void (async () => {
            try {
                ...
                showChatToast(`Offline generation failed: ${error?.message || String(error)}`, 3000);
            } finally {
    ...
    const handleOfflineEditSave = () => {
        ...
        if (!content) {
            showChatToast("Edited content cannot be empty");
            return;
        }
        ...
        if (!assistantContent) {
            showChatToast("Could not parse offline content");
            return;
        }
        if (!parsed.summary.trim()) showChatToast(`Couldn't extract <${parsed.summaryTag}> summary`);
    ...
    const handleOfflineRetryFrom = async (turnId: string) => {
        ...
        if (!retryInput) {
            showChatToast("No user input to retry for this round");
            return;
        }
        ...
        try {
            ...
            showChatToast(`Offline retry failed: ${error?.message || String(error)}`, 3000);
        } finally {
            if (!finishOfflineGenerationRun(session.id, offlineRunId)) return;
            setPendingOfflineUserText("");
            offlineGenerationInputRef.current = "";
    ...
    const handleEditResponseSave = () => {
        ...
        if (session.isGroup && editingResponseRoundId) {
            ...
            if (roundMessages.length === 0) {
                showChatToast("Couldn't find this round of group chat replies");
                setEditingResponseRoundId(null);
                setEditingResponseContent("");
                return;
            }
            ...
            if (!hasKnownGroupSenderPrefix(editedResponseContent)) {
                showChatToast("Edited group content must keep the [Character Name]: prefix");
                return;
            }
            const segments = parseGroupChatResponse(editedResponseContent, nameToId);
            if (segments.length === 0) {
                showChatToast("No editable group member prefix recognized");
                return;
            }
            ...
            if (replacementMessages.length === 0) {
                showChatToast("The edited group chat reply has no displayable content");
                return;
            }
        ...
        if (batchMessages.length === 0) {
            showChatToast("Couldn't find the original content of this reply");
            setEditingResponseBatchId(null);
            setEditingResponseRoundId(null);
            setEditingResponseContent("");
            return;
        ...
        if (normalizedParts.length === 0) {
            showChatToast("The edited reply has no displayable content");
            return;
        }
    ...
    const deleteWeixinCloudBeforeLocal = async (
        ...
        if (cloudDeletePending) {
            showChatToast("Deleting cloud records, please wait");
            return;
        }
        ...
        try {
            ...
            if (deletedCount < cloudTargetCount) {
                throw new Error("Cloud records were not fully deleted, please check your sync settings and try again.");
            }
            applyLocalDelete();
            if (successText) showChatToast(successText);
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            showChatToast(`Cloud deletion failed: ${message}`, 3500);
        } finally {
    ...
    const renderOfflineContextMenu = (turn: ChatOfflineTurn, role: OfflineActionTarget["role"]) => {
        const menu = (
            ...
            >
                <div className="flex">
                    <button onClick={() => { copyTextToClipboard(getOfflineCopyText(turn, role)); setActiveOfflineTarget(null); }} className="ctx-menu-btn">Copy</button>
                    <button onClick={() => handleOfflineEditStart(turn, role)} className="ctx-menu-btn">Edit</button>
                    <button onClick={() => void handleOfflineRetryFrom(turn.id)} className="ctx-menu-btn ctx-menu-btn-danger">Retry from here</button>
                </div>
                <div className="flex">
                    <button onClick={() => handleOfflineDeleteTurn(turn.id)} className="ctx-menu-btn ctx-menu-btn-danger">Delete</button>
                    <button onClick={() => handleOfflineDeleteTurnsFrom(turn.id)} className="ctx-menu-btn ctx-menu-btn-danger">Delete from here</button>
                </div>
            ...
            <div
                ...
                <div className="flex">
                    <button onClick={() => {
                        ...
                        setActiveMessageId(null);
                    }} className="ctx-menu-btn">Copy</button>
                    <button onClick={() => (m.role === "assistant" ? handleEditResponseStart(m) : handleEditMessageStart(m))} className="ctx-menu-btn">
                        {m.role === "assistant" && (m.rawResponseText || m.editableResponseText) ? "Edit Reply" : "Edit"}
                    </button>
                    {m.mediaType === "audio" && m.mediaData?.label && (
                        <button onClick={() => { setVoiceTextIds(prev => { const next = new Set(prev); if (next.has(m.id)) next.delete(m.id); else next.add(m.id); return next; }); setActiveMessageId(null); }} className="ctx-menu-btn">To Text</button>
                    )}
                    {m.role === "user" && (
                        <button onClick={() => handleRetractMessage(m.id)} className="ctx-menu-btn">Recall</button>
                    )}
                    {m.role === "assistant" && (
                        <button onClick={() => handleRetry(m.id)} className="ctx-menu-btn ctx-menu-btn-danger">Retry from here</button>
                    )}
                </div>
                <div className="flex">
                    <button onClick={() => { setQuotingMessage(m); setActiveMessageId(null); }} className="ctx-menu-btn">Quote</button>
                    {options?.allowMultiSelect !== false && (
                        <button onClick={() => startMultiSelectFromMessage(m)} className="ctx-menu-btn">Select</button>
                    )}
                    <button onClick={() => handleDeleteMessage(m.id)} className="ctx-menu-btn ctx-menu-btn-danger">Delete</button>
                    <button onClick={() => handleDeleteMessagesFrom(m.id)} className="ctx-menu-btn ctx-menu-btn-danger">Delete from here</button>
                </div>
                {(() => {
                    // 聊天插件注册的消息操作菜单项
    ...
    const confirmMultiDelete = useCallback(() => {
        if (multiDeleteTargetIds.length === 0) {
            showChatToast("Please select messages to delete");
            return;
        }
    ...
    const handleMultiDeleteConfirmed = () => {
        ...
        void deleteWeixinCloudBeforeLocal(targetMessages, () => {
            ...
            syncMessagesFromStorage();
            cancelMultiSelect();
            if (deletedCount > 0) showChatToast(`Deleted ${deletedCount} messages`);
        });
    };
    ...
    return (
        <div ref={wrapperRef} className={`session-${session.id} chat-room-wrapper page-shell inset-0 flex flex-col z-20`} style={chatRoomBackgroundStyle} {...(bgLoading ? { "data-loading": "" } : {})} {...(bgImageResolved ? { "data-has-bg-image": "" } : {})} {...(showSettings ? { "data-settings-open": "" } : {})}>
            ...
            {/* Fullscreen effects layer (emoji rain / confetti), doesn't intercept touch */}
                ...
                {offlineMode && (
                    <div className="chat-offline-body">
                        ...
                        {hasMoreOfflineTurns && (
                            ...
                            >
                                <span>View more offline records</span>
                        ...
                        {visibleOfflineTurns.map((turn, turnIdx) => {
                            ...
                            <div className="chat-offline-turn">
                                <div className="chat-offline-entry" data-role="user" style={offlineDisplay.userContent.trim() ? undefined : { display: "none" }}>
                                    <div className="chat-offline-label">You</div>
                                    <div
                                ...
                                <div className="chat-offline-entry" data-role="assistant">
                                    <div className="chat-offline-label-row">
                                        <div className="chat-offline-label">{session.isGroup ? (session.groupName || "Group Chat") : (character?.name || "them")}</div>
                                        {assistantHasHtmlPreview ? (
                                            <button
                                                type="button"
                                                className="chat-offline-menu-trigger"
                                                aria-label="Offline reply actions"
                                                title="Offline reply actions"
                                                onPointerDown={(e) => {
                                                    e.stopPropagation();
                                    ...
                                    {turn.reasoningText && (
                                        <button
                                            ...
                                            aria-label="View thinking process"
                                        >
                                    ...
                                    {turn.summary.trim() && (
                                        <details className="chat-offline-summary-fold">
                                            <summary>Summary ({turn.summaryTag || "summary"})</summary>
                        ...
                        {(pendingOfflineUserText || isOfflineGenerating) && (
                            <div className="chat-offline-turn">
                                ...
                                <div className="chat-offline-generating">Generating offline reply</div>
                            </div>
                        )}
                    </div>
                )}
                        ...
                        return (
                            <div key={`vc-${vcGroup.startId}`} className="flex flex-col gap-2">
                                ...
                                >
                                    ...
                                    <span>{vcGroup.callType === "video" ? "Video Call" : "Voice Call"}{vcGroup.duration ? ` ${vcGroup.duration}` : ""}{chatCount > 0 ? ` · ${chatCount} messages` : ""}</span>
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
                            ...
                            >
                                ...
                                {uiRole(msg) === "system" ? (
                                    ...
                                    <div
                                        ...
                                        {...(activeMessageId === msg.id ? { "data-active": "" } : {})}
                                    >
                                        {msg.role === "user" ? "You" : (character?.name || "They")} recalled a message
                                    ...
                                    <>
                                        {msg.role !== "user" && !isEmptyBubble && (
                                            isSilentThought ? (
                                                ...
                                                <div
                                                    ...
                                                    title={session.isGroup ? `${msg.senderName || "Group member"}'s collapsed state` : "View collapsed state"}
                                                    aria-label={session.isGroup ? `${msg.senderName || "Group member"}'s collapsed state` : "View collapsed state"}
                                                    {...(activeMessageId === msg.id ? { "data-active": "" } : {})}
                                                >
                                        ...
                                        {msg.role !== "user" && !isSilentThought && !isEmptyBubble && hasFoldedPanel && (
                                            <button
                                                ...
                                                title="View collapsed state"
                                                aria-label="View collapsed state"
                                            >
                            ...
                            {hasFoldedPanel && expandedMonologueId === msg.id && (
                                <div className="chat-thought-card">
                                    ...
                                    <div className="chat-thought-title">
                                        💭 {renderMsg.innerMonologue ? "Inner Monologue" : "Status Bar"}
                                    </div>
                                    {/* State values panel */}
                                    ...
                                    <div className="chat-thought-sig">
                                        — {session.isGroup ? (msg.senderName || "Group member") : (character?.name || "Them")}
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="chat-multi-select-summary">
                        ...
                        <span>
                            {multiDeleteTargetIds.length > selectedMessageIds.size
                                ? `Will delete ${multiDeleteTargetIds.length}, including hidden history`
                                : `Will delete ${multiDeleteTargetIds.length}`}
                        </span>
                    </div>
                    <button
                        type="button"
            ...
            <ChatTextInputBar
                ...
                characterName={character?.name || "them"}
                characterId={session.contactId}
            ...
            {showConfirmMultiDelete && (
                <ConfirmDialog
                    ...
                    message={
                        multiDeleteTargetIds.length > selectedMessageIds.size
                            ? `This will delete the selected messages, along with any hidden history between adjacent selected messages. ${multiDeleteTargetIds.length} messages will be deleted and cannot be recovered.`
                            : `This will delete the ${multiDeleteTargetIds.length} selected messages. This cannot be undone.`
                    }
                    icon={AlertCircle}
                    variant="danger"
                    confirmLabel="Delete"
                ...
                <div className="chat-settings-layer absolute inset-0 z-50">
                    <ChatSettingsPanel
                        ...
                        onOfflineHistoryCleared={() => {
                            ...
                            setEditingOfflineTarget(null);
                            setEditingOfflineContent("");
                            showChatToast("Offline chat history cleared");
                        }}
                    ...
                    <div className="modal-sheet chat-reasoning-sheet" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="chat-reasoning-sheet-header">
                            <button
                                ...
                                aria-label={reasoningTranslation ? "Hide translation" : "Translate thinking process"}
                                title={reasoningTranslation ? "Hide translation" : "Translate thinking process"}
                            >
                                {reasoningTranslating
                                    ? <Loader2 size={18} strokeWidth={2} className="animate-spin" />
                                    : <Languages size={18} strokeWidth={2} {...(reasoningTranslation ? { color: "var(--c-icon-active)" } : {})} />}
                            </button>
                            <span className="chat-reasoning-sheet-title">Thinking process</span>
                            <button
                                type="button"
                                className="chat-reasoning-sheet-close"
                                onClick={() => setReasoningSheetText(null)}
                        ...
                        <div className="chat-reasoning-sheet-body">
                            ...
                            {reasoningTranslation && (
                                <div className="chat-reasoning-view-switch">
                                    {([["zh", "Chinese"], ["orig", "Original"], ["both", "Both"]] as const).map(([mode, text]) => (
                                        <button
                                            key={mode}
                                            type="button"
                        ...
                        <div className="flex items-center justify-between gap-3">
                            <div className="flex flex-col gap-1">
                                <span className="menu-label">
                                    {editingOfflineTarget.role === "user" ? "Edit Offline Input" : "Edit Offline Reply"}
                                </span>
                                <span className="menu-desc !mt-0">
                                    {editingOfflineTarget.role === "user"
                                        ? "Saving will update this round's offline history"
                                        : "Saving will re-parse the content and summary, and update the short-term memory event stream"}
                                </span>
                            </div>
                            <button
            ...
            {editingMessageId && (
                <div className="chat-html-overlay" onClick={() => { setEditingMessageId(null); setEditingContent(""); }}>
                    ...
                    >
                        <div className="flex items-center justify-between gap-3">
                            <div className="flex flex-col gap-1">
                                <span className="menu-label">{editingSystemInstruction ? "Edit System Instruction" : "Edit Message"}</span>
                                <span className="menu-desc !mt-0">{editingSystemInstruction ? "Saving will update the following context at this position" : "Saving will sync the chat history and following context"}</span>
                            </div>
                            <button
            ...
            {(editingResponseBatchId || editingResponseRoundId) && (
                <div className="chat-html-overlay" onClick={() => { setEditingResponseBatchId(null); setEditingResponseRoundId(null); setEditingResponseContent(""); }}>
                    ...
                    >
                        <div className="flex items-center justify-between gap-3">
                            <div className="flex flex-col gap-1">
                                <span className="menu-label">Edit This Reply</span>
                                <span className="menu-desc !mt-0">Saving will re-split this AI reply based on the new edited text</span>
                            </div>
                            <button
            ...
            {cloudDeletePending && (
                <div className="modal-overlay" data-ui="modal" role="alertdialog" aria-modal="true" aria-label="Deleting cloud records">
                    <div className="modal-dialog" data-ui="modal-dialog" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex flex-col items-center gap-2 text-center">
                            <h3 className="modal-title">Deleting Cloud Records</h3>
                            <p className="menu-desc !mt-0">
                                Deleting {cloudDeletePending.count} WeChat cloud records, please don't close the page.
                            </p>
                            <p className="menu-desc !mt-0">
                                If this takes more than {Math.round(WEIXIN_CLOUD_DELETE_TIMEOUT_MS / 1000)} seconds, it will be automatically marked as failed.
                            </p>
                        </div>
                    </div>
                </div>
