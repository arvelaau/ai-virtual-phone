...
import { Maximize2, ReceiptText, RefreshCw } from "lucide-react";
    ...
    if (!registration) {
        return (
            <div className="bubble bubble-text opacity-60">
                <span className="ts-12">[Plugin message: {kind}] (plugin not enabled)</span>
            </div>
        );
    }
    ...
    return (
        <div className="chat-html-inline" data-chat-html-inline="" data-variant={variant}>
            <iframe
                ...
                srcDoc={srcDoc}
                title="AI-generated interactive content"
                style={{ height }}
            />
            {allowFullscreen ? (
                <button
    ...
    return (
        ...
        >
            ...
            <div className="ui-media-footer px-4 py-1.5 ts-11" {...(isDeclined ? { "data-status": "declined" } : {})}>
                <span>WeChat Red Envelope</span>
                {totalRecipients > 1 && claimedBy.length > 0 && (
                    <span className="ml-1 opacity-70">· {claimedBy.length}/{totalRecipients} claimed</span>
                )}
            </div>
        </div>
    );
...
function TransferBubble({ msg, charName, userName, onShowDetail }: {
    ...
    return (
        ...
        >
            ...
            {d?.recipientName && (
                <div className={`px-4 py-1 ts-12 ui-text-white-70 ${bgClass}`}>To {d.recipientName}</div>
            )}
            <div
                className="ui-media-footer px-4 py-2 ts-12 flex justify-between items-center"
                {...(isDeclined ? { "data-status": "declined" } : {})}
            >
                <span>WeChat Transfer</span>
                {isReceived && <span>Received</span>}
                {isDeclined && <span>Returned</span>}
            </div>
        </div>
    );
}
...
function PaymentRequestBubble({ msg, charName, userName, onShowDetail }: {
    ...
    const requester = msg.role === "user" ? (userName || "You") : (msg.senderName || charName || "the other person");
    ...
    const itemsText = d?.paymentRequestItemsText || (d?.paymentRequestItems || [])
        ...
        .join("; ");
    const statusText = isPaid ? "Paid" : isDeclined ? "Declined" : "Pending";
...
function AppCardBubble({ msg, characterId, characterName }: { msg: ChatMessage; characterId?: string; characterName?: string }) {
    ...
    // Chat's own fallback chain is richer than story/offline's (they have no per-message
    // directive metadata to fall back through) -- computed here, passed to the shared view as
    // plain override strings.
    const fallbackTitle = d?.appCardTitle || d?.label || appName;
    const fallbackBody = d?.appDirectiveId
        ? (d?.appCardBody || "")
    ...
    return (
        <AppCardView
            ...
            appId={d?.appId}
            onOpen={openApp}
            pinContext={characterId ? {
                sourceMode: "chat",
                characterId,
                characterName: characterName || "",
                // Chat has no per-message summary field -- fall back to the visible text,
                // same fallback the open-app launchContext above already uses.
                summary: d?.appCardSummary || msg.content,
                cardAppId: d?.appId,
                cardAppName: d?.appName,
                messageId: msg.id,
                sessionId: msg.sessionId,
            } : undefined}
        />
...
function ContactCardBubble({ msg, characterId }: { msg: ChatMessage; characterId?: string }) {
    ...
    const recommenderName = useMemo(
        () => loadCharacters().find(c => c.id === characterId)?.name || "the other person",
        [characterId],
    );
            ...
            <div className="chat-contact-card" onClick={handleClick} role="button">
                <div className="chat-contact-card-main">
                    ...
                    <div className="chat-contact-card-info">
                        <div className="chat-contact-card-name">{contactName || "Contact"}</div>
                        <div className="chat-contact-card-sub">
                            {resolved.character
                                ? `WeChat ID: ${resolved.character.wechatID || resolved.character.id.slice(0, 10)}`
                                : "Tap to view"}
                        </div>
                    </div>
                    {resolved.isContact && <span className="chat-contact-card-badge">Added</span>}
                </div>
                <div className="chat-contact-card-footer">Personal Card</div>
            </div>
...
function GiftBubble({ msg }: { msg: ChatMessage }) {
    const d = msg.mediaData;
    const title = d?.giftName || d?.label || "Gift";
    const recipient = d?.recipientName;
    const merchant = d?.giftMerchantLabel || "Shopping Order";
            ...
            >
                <div className="relative z-[1] min-h-[298px] flex flex-col">
                    ...
                    <div className="chat-gift-card-grid mt-5 grid grid-cols-2 gap-x-4 gap-y-3">
                        {recipient && (
                            <GiftInfoCell label="Recipient" value={recipient} strong />
                        )}
                        <GiftInfoCell label="ID" value={`G-${serial}`} />
                        <GiftInfoCell label="Source" value={merchant} />
                        <GiftInfoCell label="Gift Value" value={d?.giftPriceLabel || "A Thoughtful Gift"} />
                        {sentLabel && <GiftInfoCell label="Sent" value={sentLabel} />}
                    </div>
        ...
        >
            <div className="modal-dialog chat-generated-image-prompt-dialog" data-ui="modal-dialog" onClick={e => e.stopPropagation()}>
                <div className="modal-header" data-ui="modal-header">
                    <h3 className="modal-title">Regenerate Image</h3>
                </div>
                <div className="modal-body chat-generated-image-prompt-body" data-ui="modal-body">
                    <textarea
                        ...
                        disabled={busy}
                    />
                    {error && <div className="chat-generated-image-retry-error">Generation failed: {error}</div>}
                </div>
                <div className="modal-footer" data-ui="modal-footer">
                    <button className="ui-btn ui-btn-ghost" onClick={onCancel}>Cancel</button>
                    <button
...
function ImageBubble({
    ...
    const handleRetry = useCallback(() => {
        ...
        if (!nextDescription) {
            setRetryError("Prompt cannot be empty");
            return;
        }
        ...
        return (
            <div className="chat-photo-card chat-photo-card--pending w-[180px] aspect-square rounded-none">
                <div className="chat-photo-card-pending-inner">
                    ...
                    <div className="chat-photo-card-pending-text">Receiving image...</div>
                </div>
            </div>
        );
    }
    return (
        <div className="chat-generated-image-retry-stack">
            ...
            {retryError && <div className="chat-generated-image-retry-error">Generation failed: {retryError}</div>}
...
function DiceBubble({ msg }: { msg: ChatMessage }) {
    ...
    return (
        <div className="dice-bubble3d" aria-label={`Dice showing ${face}`}>
...
export function MediaDetailModal({ msg, userName, groupSize, onAccept, onClose }: MediaDetailModalProps) {
    ...
    const senderDisplay = isFromUser ? userName : (d?.senderName || msg.senderName || "the other person");
    ...
    const handlePaymentRequestAccept = () => {
        ...
        if (safeAmount <= 0) {
            setPaymentError("Invalid amount, unable to pay on their behalf.");
            return;
        }
        ...
        if (!result.ok || !result.transaction) {
            setPaymentError(result.error ?? "Insufficient balance, unable to pay on their behalf.");
            return;
        }
        const updatedData = {
            ...d,
    ...
    if (isRedPacket) {
        ...
        if (isTransfer) {
            if (isReceived) statusText = "Received";
            else if (isDeclined) statusText = "Returned";
            else if (isFromUser) statusText = "Transfer you sent";
            else if (!isRecipient) statusText = `To ${d?.recipientName}`;
        } else if (isPaymentRequest) {
            if (isPaid) statusText = "Paid";
            else if (isDeclined) statusText = "Declined";
            else if (isFromUser) statusText = "Payment request you sent";
        }
    }
        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="media-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className={`media-modal-header ${gradientClass}`}>
                    ...
                    <div className="media-modal-sub">From {senderDisplay}</div>
                    {isTransfer && d?.recipientName && (
                        <div className="media-modal-sub">To {d.recipientName}</div>
                    )}
                    {isRedPacket && totalRecipients > 1 && (
                        <div className="media-modal-sub">{totalRecipients} total · Lucky Red Envelope · {claimedBy.length}/{totalRecipients} claimed</div>
                    )}
                </div>
                ...
                <div className="media-modal-body">
                    ...
                    {canClaimRedPacket ? (
                        <div className="media-modal-actions">
                            <button onClick={handleRedPacketAccept} className="ui-btn ui-btn-redpacket">Claim</button>
                            <button onClick={handleRedPacketDecline} className="ui-btn ui-btn-outline">Return</button>
                        </div>
                    ) : canActTransfer ? (
                        <div className="media-modal-actions">
                            <button onClick={handleTransferAccept} className="ui-btn ui-btn-transfer">Accept</button>
                            <button onClick={handleTransferDecline} className="ui-btn ui-btn-outline">Return</button>
                        </div>
                    ) : canActPaymentRequest ? (
                        <div className="media-modal-actions">
                            <button onClick={handlePaymentRequestAccept} className="ui-btn ui-btn-transfer">Pay for them</button>
                            <button onClick={handlePaymentRequestDecline} className="ui-btn ui-btn-outline">Decline</button>
                        </div>
                    ) : statusText ? (
...
function MediaFileBubble({
    ...
    const handleRegenerateImage = useCallback(() => {
        ...
        if (!nextDescription) {
            setImageRetryError("Prompt cannot be empty");
            return;
        }
    ...
    if (expired) {
        return (
            <div className="chat-media-file-card chat-media-file-generic">
                <span className="chat-media-file-title" style={{ opacity: 0.5 }}>File expired</span>
            </div>
        );
    }
    ...
    if (!url && isMediaStoreRef(rawUrl)) {
        return (
            <div className="chat-media-file-card chat-media-file-generic">
                <span className="chat-media-file-title" style={{ opacity: 0.5 }}>Loading...</span>
            </div>
        );
    }

    if (fileType === "image" && !url) {
        const fallbackTitle = msg.mediaData?.label?.trim() || title || "Image";
        return (
            <div className="chat-media-file-card chat-media-file-generic">
                    ...
                    <div className="chat-media-file-header">
                        ...
                        <div className="chat-media-file-info">
                            ...
                            <div className="chat-media-file-time">
                                {duration > 0 ? `${formatTime(progress * duration)} / ${formatTime(duration)}` : "Loading..."}
                            </div>
                        </div>
                    </div>
    ...
    if (fileType === "image" && url) {
        ...
        return (
            <div className="chat-generated-image-retry-stack">
                ...
                />
                {imageRetryError && <div className="chat-generated-image-retry-error">Generation failed: {imageRetryError}</div>}
...
function MusicShareBubble({ msg, onPlay }: { msg: ChatMessage; onPlay?: (title: string, artist?: string) => void }) {
    const title = msg.mediaData?.musicTitle || "Unknown Song";
...
function XiaohongshuShareBubble({ msg }: { msg: ChatMessage }) {
    ...
    const title = data?.xiaohongshuTitle || "Xiaohongshu Post";
    const author = data?.xiaohongshuAuthor || "Xiaohongshu User";
    ...
    return (
        <div className="chat-xhs-share-card">
            <div className="chat-xhs-share-head">
                ...
                <span>{data?.xiaohongshuNoteType === "video" ? "Video Post" : "Xiaohongshu Post"}</span>
            </div>
...
function VoiceMessageBubble({ msg, characterId, onUpdate, defaultTranslationExpanded = false }: { msg: ChatMessage; characterId?: string; onUpdate?: (m: ChatMessage) => void; defaultTranslationExpanded?: boolean }) {
    ...
    const text = msg.mediaData?.label || "Voice Message";
    const bilingual = splitBilingualText(text);
