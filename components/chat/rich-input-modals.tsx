...
export function PhotoInputModal({ onSend, onClose }: PhotoInputModalProps) {
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            ...
            >
                <div className="ts-16 font-semibold text-center text-[var(--c-text)]">Send Photo</div>
                <div
                ...
                >
                    {imageDataUrl ? (
                        ...
                        <div className="flex flex-col items-center gap-2 py-6">
                            ...
                            <span className="ts-12 text-[var(--c-icon)]">Tap to upload photo</span>
                        </div>
                    )}
                    <input
                        ref={fileInputRef}
...
export function RedPacketModal({ mode, isGroup, onSend, onClose }: RedPacketModalProps) {
    ...
    const title = isRedPacket ? "Send Red Envelope" : "Transfer";
        ...
        <div className="modal-overlay" onClick={onClose}>
            ...
            >
                ...
                <div className="p-5 flex flex-col gap-3.5">
                    <div>
                        <div className="ts-12 text-[var(--c-icon)] mb-1.5">Amount</div>
                    ...
                    {isRedPacket && isGroup && (
                        <div>
                            <div className="ts-12 text-[var(--c-icon)] mb-1.5">Count</div>
                            <input
                                value={count}
                    ...
                    <div>
                        <div className="ts-12 text-[var(--c-icon)] mb-1.5">
                            {isRedPacket ? "Message" : "Note"}
                        </div>
                        <input
                            value={label}
                            onChange={e => setLabel(e.target.value)}
                            placeholder={defaultLabel || "Add a note"}
                            className="ui-input w-full"
                        />
                    </div>
                    <div className="flex gap-3 mt-1">
                        ...
                        >{isRedPacket ? "Put Money In" : "Confirm Transfer"}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div
                ...
                className="modal-dialog"
            >
                <div className="ts-16 font-semibold text-center text-[var(--c-text)]">Share Location</div>
                <div
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div onClick={e => e.stopPropagation()} className="modal-dialog">
                <div className="ts-16 font-semibold text-center text-[var(--c-text)]">Text Image</div>
                <div className="w-full h-[100px] rounded-xl flex items-center justify-center ui-placeholder-gradient">
                    <span className="ts-13 text-[var(--c-text)] opacity-60 px-4 text-center leading-relaxed">
                        {text.trim() || "Enter text to generate an image"}
                    </span>
                </div>
                <textarea
                    value={text}
                ...
                />
                <div className="flex gap-3 w-full">
                    <button onClick={onClose} className="ui-btn ui-btn-ghost ui-btn-bordered-ghost flex-1">Cancel</button>
                    <button
...
export function SystemInstructionModal({ onSend, onClose }: SystemInstructionModalProps) {
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div onClick={e => e.stopPropagation()} className="modal-dialog">
                <div className="ts-16 font-semibold text-center text-[var(--c-text)]">Inject System Instruction</div>
                <label className="w-full flex flex-col gap-2">
                    <span className="menu-label">Instruction Content</span>
                    <textarea
                        value={text}
                        onChange={e => setText(e.target.value)}
                        onKeyDown={e => {
...
export function VoiceRecordModal({ characterId, onSend, onClose }: VoiceRecordModalProps) {
    ...
    const startRecording = useCallback(async () => {
        ...
        const stt = createSTTSession({
            ...
            onNoSpeech: () => { setInterim("No speech detected"); setState("idle"); recorderRef.current?.stop(); showSttCompatibilityWarning(); },
        }, "zh-CN");
        ...
        if (!navigator.mediaDevices?.getUserMedia) {
            setInterim("Unable to access microphone");
            showSttCompatibilityWarning();
            return;
        }
            ...
            <div onClick={e => e.stopPropagation()} className="modal-dialog relative">
                {!androidTextInputOnly && (
                    <button
                        ...
                        title={inputMode === "voice" ? "Text Input" : "Voice Input"}
                    >
                        {inputMode === "voice" ? "Aa" : (
                ...
                <div className="ts-16 font-semibold text-center text-[var(--c-text)]">Voice Message</div>
                <div className="w-full min-h-[80px] flex flex-col items-center justify-center gap-3 py-4">
                    ...
                    {(state === "recording" || state === "processing") && (
                        <span className="ts-13 text-[var(--c-icon)]">{interim || "Tap to stop recording"}</span>
                    )}
                </div>
                <div className="flex gap-3 w-full">
                    <button onClick={onClose} className="ui-btn ui-btn-ghost ui-btn-bordered-ghost flex-1">Cancel</button>
                ...
                {!androidTextInputOnly && showSttWarning && (
                    <CallSttWarningDialog
                        title="Voice Message Notice"
                        message="Your browser may not support speech recognition, microphone permission may not be enabled, or the microphone has no input. You can tap Aa in the top right to switch to text input for the voice message."
