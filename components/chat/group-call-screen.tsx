...
export function GroupCallScreen({ type, session, characters, onEnd, initiator = "user", initiatorName }: GroupCallScreenProps) {
    ...
    useEffect(() => {
        ...
        userNameRef.current = ui?.name || "You";
    ...
    const stateLabel = (): string => {
        switch (callState) {
            case "CONNECTING": return initiator === "character" ? "Incoming call..." : "Calling...";
            case "IDLE": return isMuted ? "Muted" : "In call";
        ...
        try {
            ...
            if (stateRef.current !== "ENDED") {
                setSubtitles(prev => [...prev, {
                    ...
                    text: `⚠️ ${error?.message || "Failed to send"}`,
                }]);
                setCallState("IDLE");
            }
        }
            ...
            >
                <div className="call-text-input-shell">
                    <input
                        ...
                        placeholder={callState === "IDLE" ? "Type what you want to say..." : "Wait for them to finish..."}
    ...
    const renderControls = () => {
        if (callState !== "ENDED" && callState !== "CONNECTING") {
            if (androidTextInputOnly) {
                return (
                    <button onClick={handleHangup} className={`ui-call-btn ${isVideo ? "ui-call-btn-sm" : ""} ui-call-btn-danger`} aria-label="Hang up">
            ...
            return (
                <>
                    ...
                    <button
                        ...
                        aria-label={androidTextInputOnly ? "Text input" : inputMode === "voice" ? "Switch to text input" : "Switch to voice input"}
                        title={androidTextInputOnly ? "Use text input on Android browsers" : inputMode === "voice" ? "Switch to text input" : "Switch to voice input"}
                    >
                        {inputMode === "text" ? (
        ...
        return <div className="ts-14 opacity-70">Call ended</div>;
    };
        ...
        >
            ...
            <div className="gcall-body voicecall-controls">
                ...
                <div className="gcall-topbar">
                    <div className="gcall-topbar-title">Group Voice Call ({characters.length + 1} people)</div>
                    <div
