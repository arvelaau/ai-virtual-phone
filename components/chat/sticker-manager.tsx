...
export function StickerManager({ onBack }: { onBack: () => void }) {
    ...
    return (
        <PageShell title="Sticker Manager" onBack={onBack} className="absolute inset-0 z-[100]">
            <div className="px-5 pt-4 pb-8 h-full overflow-y-auto">
                <div className="grid grid-cols-2 gap-4">
                        ...
                        <div key={pack.id} className="relative group">
                            ...
                            >
                                ...
                                <div className="flex flex-col w-full gap-0.5">
                                    ...
                                    <span className="ts-12 text-[var(--c-text)] opacity-70">{pack.stickers.length === 0 ? "Empty album" : `${pack.stickers.length} stickers`}</span>
                                </div>
                            ...
                            <button
                                type="button"
                                aria-label={`Delete sticker pack ${pack.name}`}
                    ...
                    >
                        ...
                        <span className="ts-14 font-medium text-[var(--c-text)]">New Album</span>
                    </button>
                </div>
            </div>
            ...
            {deletingPackId && createPortal(
                <ConfirmDialog
                    ...
                    message="All stickers in this pack will be cleared after deletion. Are you sure you want to delete it?"
                    variant="danger"
                    confirmLabel="Delete"
        ...
        <div className="modal-overlay" data-ui="modal" onClick={onCancel}>
            <div className="modal-dialog" onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <h3 className="modal-title">Create Sticker Pack</h3>
                </div>
                ...
                <div className="modal-body">
                    <div className="flex flex-col gap-4 text-left w-full">
                        ...
                        {characters.length > 0 && (
                            <div className="flex flex-col gap-1">
                                <label className="menu-desc ml-1">Apply to Characters</label>
        ...
        <PageShell title={currentPack.name} onBack={onBack} className="absolute inset-0 z-[100]">
            <div className="flex flex-col h-full bg-[var(--c-page-body-bg)]">
                ...
                <div className="px-6 pt-5 pb-2 shrink-0">
                    <div className="text-[calc(12px*var(--app-text-scale,1))] font-bold text-[var(--c-text)] opacity-60 uppercase mb-3 px-1 tracking-[0.1em]">Smart Character Binding</div>
                ...
                <div className="flex-1 mt-4 bg-[var(--c-card)] rounded-t-[32px] p-6 shadow-[0_-8px_32px_rgba(0,0,0,0.02)] border-t border-[var(--c-card-border)]/30 min-h-0 overflow-y-auto">
                    <div className="flex items-center justify-between mb-5 px-1">
                        <div className="text-[calc(18px*var(--app-text-scale,1))] font-bold text-[var(--c-text-title)]">Sticker Library <span className="text-[var(--c-text)] opacity-50 font-medium ml-1 text-base">{currentPack.stickers.length}</span></div>
                        <button
                            type="button"
                    ...
                    <div className="grid grid-cols-3 gap-x-4 gap-y-6 pb-20">
                        ...
                        <div className="flex flex-col items-center">
                            ...
                            >
                                ...
                                <span className="ts-11 font-medium">Add</span>
                            </button>
                        </div>
                    </div>
                </div>
...
function AddStickerDialog({
    ...
    return (
        <div className="modal-overlay" data-ui="modal" onClick={onCancel}>
            <div className="modal-dialog" onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <h3 className="modal-title">Add Sticker</h3>
                </div>
                    ...
                    <div className="flex flex-col gap-4 text-left w-full">
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                ...
                                value={name}
                                onChange={e => setName(e.target.value)}
                                placeholder="The AI will use this name to send the sticker"
                                className="ui-input"
                            />
                        </div>

                        <div className="flex flex-col gap-1">
                            <label className="menu-desc ml-1">Image URL</label>
                            <input
                                type="text"
                                value={url}
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            {previewSrc ? (
                                ...
                                >
                                    <ImagePlus size={28} />
                                    <span className="ts-13">Choose Image</span>
                                </button>
                            )}
                        </div>
                    </div>
...
function getStickerBaseName(filename: string): string {
    return filename.replace(/\.[^.]+$/, "").trim() || "Sticker";
}
...
function getStickerNameFromUrl(url: string, index: number): string {
    try {
        ...
        return getStickerBaseName(filename) || `Sticker ${index + 1}`;
    } catch {
...
function BatchAddStickerDialog({
    ...
    const addUrls = () => {
        ...
        if (parsed.length === 0) {
            setUrlError("No valid image URLs found");
            return;
        }
        setRows(prev => [
            ...prev,
    ...
    const numberNames = () => setRows(prev => prev.map((r, i) => ({ ...r, name: `Sticker ${i + 1}` })));
    ...
    const rowError = (r: BatchRow): string | null => {
        ...
        if (existingNames.current.has(k)) return "Duplicate name with an existing sticker";
        if ((nameCounts.get(k) ?? 0) > 1) return "Duplicate name in this batch";
        return null;
    };
    ...
    return (
        <div className="modal-overlay sticker-batch-import-overlay" data-ui="modal" onClick={adding ? undefined : onCancel}>
            <div className="modal-dialog sticker-batch-import-dialog" onClick={e => e.stopPropagation()} style={{ width: "min(440px, 92vw)" }}>
                <div className="modal-header">
                    <h3 className="modal-title">Batch Add Stickers{rows.length > 0 ? ` · ${rows.length}` : ""}</h3>
                </div>
                ...
                <div className="modal-body">
                    ...
                    <div className="flex flex-col gap-3 text-left w-full">
                        {rows.length === 0 ? (
                            ...
                            >
                                <ImagePlus size={30} />
                                <span className="ts-13 font-medium">Choose multiple images</span>
                                <span className="ts-11 opacity-60">Or paste a URL list below</span>
                            </button>
                        ) : (
                            <div className="flex items-center flex-wrap gap-2">
                                <button className="ui-chip" onClick={() => fileInputRef.current?.click()}>+ Add more</button>
                                <button className="ui-chip" onClick={resetToFilenames}>Name by source</button>
                                <button className="ui-chip" onClick={numberNames}>Number sequentially</button>
                            </div>
                        )}

                        <div className="flex flex-col gap-1.5">
                            <label className="menu-desc ml-1">Batch URLs</label>
                            <textarea
                                value={urlText}
                                onChange={e => { setUrlText(e.target.value); setUrlError(null); }}
                                placeholder={"Sticker: https://example.com/a.jpg\nCute https://example.com/b.gif"}
                                className="ui-input"
                                rows={4}
                                style={{ resize: "vertical", minHeight: 88 }}
                            />
                            <div className="flex items-center justify-between gap-2">
                                <span className="ts-11 opacity-60">One per line, supports "Name: URL" or "Name URL"</span>
                                <button type="button" className="ui-chip" onClick={addUrls} disabled={adding || !urlText.trim()}>Add URLs</button>
                            </div>
                                        ...
                                        <div key={r.id} className="flex items-center gap-3">
                                            ...
                                            <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                                                ...
                                                <span className="ts-11 ml-1" style={{ color: err ? "var(--c-danger)" : "var(--c-text)" }}>
                                                    {err || (r.source === "url" ? "URL image" : "Local image")}
                                                </span>
                                            </div>
                                            <button
                                                type="button"
                ...
                <div className="modal-footer">
                    <button className="ui-btn ui-btn-ghost" onClick={onCancel} disabled={adding}>Cancel</button>
                    <button
                        className="ui-btn ui-btn-primary"
                        onClick={handleSubmit}
                        disabled={!allValid || adding}
                    >
                        {adding ? `Adding ${progress}%` : rows.length > 0 ? `Add All (${readyCount}/${rows.length})` : "Add All"}
                    </button>
                </div>
            </div>
        </div>
