    ...
    return (
            ...
            footer={showCompose ? (
                ...
                <div className="feed-comment-modal-layer" data-ui="modal">
                    ...
                    <div
                        ...
                        role="dialog"
                        aria-modal="true"
                        aria-label={activeComposer.replyTo ? `Reply to ${activeComposer.replyTo.name}` : "Add a Comment"}
                    >
                        <div className="feed-comment-modal-title">
                            {activeComposer.replyTo ? `Reply to ${activeComposer.replyTo.name}` : "Add a Comment"}
                        </div>
                        <textarea
                            ...
                            placeholder={activeComposer.replyTo ? `Reply to ${activeComposer.replyTo.name}` : "Say something..."}
        ...
        >
                    ...
                    >
                        ...
                        <div className="feed-profile-avatar w-[72px] h-[72px] rounded-full border-[3px] border-[var(--c-page-body-bg)] bg-[var(--c-input)] overflow-hidden flex items-center justify-center translate-x-[2px] pointer-events-auto">
                            {userIdentity?.avatarUrl ? (
                                ...
                                <span className="feed-profile-avatar-fallback ts-24 text-[var(--c-icon)] font-bold">{(userIdentity?.name ?? "Me")[0]}</span>
                            )}
                        </div>
                        
                        {/* Name and Flex Data */}
                        <div className="feed-profile-info flex flex-col gap-1 mt-3 ml-[6px] pointer-events-auto">
                            <span className="feed-profile-name ts-20 font-bold text-[var(--c-text-title)]">{userIdentity?.name ?? "Me"}</span>
                            <div className="feed-profile-stats flex gap-4 ts-13 text-[var(--c-icon)] font-medium mt-[2px]">
                                <span className="feed-profile-stat"><strong className="feed-profile-stat-value text-[var(--c-text-title)]">128</strong> Following</span>
                                <span className="feed-profile-stat"><strong className="feed-profile-stat-value text-[var(--c-text-title)]">12.4K</strong> Followers</span>
                                <span className="feed-profile-stat"><strong className="feed-profile-stat-value text-[var(--c-text-title)]">8.2M</strong> Likes & Favorites</span>
                            </div>
                ...
                {unreadNotifs.length > 0 && (
                    ...
                    >
                        {unreadNotifs.length} new comments/replies/likes
                    </button>
                )}
            ...
            {confirmDeleteId && (
                <ConfirmDialog
                    ...
                    message="This cannot be undone. Comments will be deleted too."
                    icon={AlertCircle}
                    variant="danger"
                    confirmLabel="Delete"
                    cancelLabel="Cancel"
            ...
            {showNotifModal && (
                <div className="modal-overlay" onClick={() => { setShowNotifModal(false); saveMomentsLastSeen(); setUnreadNotifs([]); }}>
                    <div className="modal-dialog" onClick={e => e.stopPropagation()} style={{ maxHeight: "60vh", overflow: "auto" }}>
                        <div className="ts-16 font-semibold text-center text-[var(--c-text)] mb-3">New Messages</div>
                        {unreadNotifs.length === 0 ? (
                            <div className="ts-14 text-[var(--c-icon)] text-center py-4">No new messages</div>
                        ) : (
                                ...
                                {unreadNotifs.map((n, i) => (
                                    <div key={i} className="flex flex-col gap-1 px-1">
                                        <span className="ts-13 text-[var(--c-text)]">
                                            ...
                                            {n.type === "comment" ? " commented on your post: " : n.type === "reply" ? " replied to you: " : " liked your post"}
                                        </span>
