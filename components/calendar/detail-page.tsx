...
const STRIP_RADIUS = 8;   // Week strip: +/- 8 weeks
const DAY_RADIUS = 21;    // Timeline: +/- 21 days (enough for 3 pages of 7-day view, and stays week-aligned)
...
/** Column-assignment layout for overlapping events. */
...
export function CalendarDetailPage({
  ...
  /** Cycle check-in row (user view only), rendered below the week strip. */
  cyclePanel: ReactNode;
  /** How many days the timeline shows per page (1/2/3/5/7). */
  daysPerPage: number;
  /** Opens the "days per page" picker. */
  onOpenDaysPicker: () => void;
  /** Cycle settings entry point (user view only), rendered in the top-right corner. */
  ...
  // ── Selection highlight (black/red dot + two-day gray pill), positioned imperatively ──
  ...
  // Whenever the data range (weeks/days) changes, re-position every scroller and highlight.
  useEffect(() => {
    requestAnimationFrame(() => {
      ...
      if (tlVRef.current && tlVRef.current.scrollTop === 0) {
        // Default view starts at 08:00; scroll up to see earlier hours.
  ...
  // Whenever days-per-page changes, re-align the selected day to the new column width.
  ...
  // Horizontal timeline scroll: the gray pill follows frame-by-frame; an index change updates the selection immediately.
  ...
  // Current-time red line (refreshed every minute).
  ...
  const backLabel = MONTH_ABBR[selectedDate.getMonth()];
  ...
  return (
    <div className="calendar-page calendar-page-detail">
      <div className="calendar-apple-topbar">
        ...
        <button type="button" className="calendar-icon-btn" onClick={onOpenDaysPicker} aria-label="Days per page">
          <Columns2 size={17} />
        </button>
        {onOpenCycleSettings ? (
          <button type="button" className="calendar-icon-btn" onClick={onOpenCycleSettings} aria-label="Cycle settings">
            <Settings2 size={17} />
          </button>
        ) : null}
      </div>
            ...
            {Array.from({ length: 7 }, (_, i) => {
              ...
              return (
                <button
                  ...
                  aria-label={`${MONTH_ABBR[d.getMonth()]} ${d.getDate()}`}
                >
      ...
      {/* Column head row: fixed above the timeline, scrolls horizontally in sync with it. */}
            ...
            return (
              <div key={iso} className="calendar-tl-day-head" data-today={iso === todayIso ? "true" : undefined}>
                {daysPerPage >= 5 ? (
                  ...
                  <>
                    <b>{MONTH_ABBR[d.getMonth()]} {d.getDate()} &ndash; {WEEKDAY_SHORT[d.getDay()]}</b>
                  ...
                  <div className="calendar-tl-body">
                    ...
                    {positioned.map(pos => (
                      ...
                      >
                        ...
                        <span>
                          {pos.item.startTime}&ndash;{pos.item.endTime}
