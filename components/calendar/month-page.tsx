...
type MonthBlock = {
  ym: string;           // "2026-7" (month is 0-based)
  year: number;
...
/** Builds month-calendar data for today +/- 12 months (the lunar calendar is only computed once per day). */
...
export function CalendarMonthPage({
  ...
  // Top glass overlay height -> scroll area's padding-top.
  useEffect(() => {
  ...
  // Scroll to the current month on first load (once the header height is known).
  ...
  return (
    <div className="calendar-page calendar-page-month">
      <div className="calendar-month-scroll hide-scrollbar" ref={scrollRef} onScroll={handleScroll} style={{ paddingTop: headerH }}>
        {months.map(block => (
          <div key={block.ym} className="calendar-month-block" data-ym={block.ym}>
            ...
            >
              <strong data-current={block.ym === todayYm ? "true" : undefined}>{MONTH_LABELS[block.month]}</strong>
            </div>
                ...
                {week.map(cell => {
                  ...
                  return (
                    <button
                      ...
                      aria-label={`${MONTH_LABELS[block.month]} ${cell.day}${items?.length ? `, ${items.length} event${items.length === 1 ? "" : "s"}` : ""}`}
      ...
      {/* Top glass overlay: weekday row and above, calendar content shows through beneath. */}
      <div className="calendar-month-header" ref={headerRef}>
        <div className="calendar-apple-topbar">
          <button type="button" className="calendar-pill-btn calendar-back-btn" onClick={onClose} aria-label="Back to home">
            <ChevronLeft size={19} />
            {titleYear}
          </button>
          <span className="calendar-topbar-space" />
          <button type="button" className="calendar-icon-btn" onClick={onOpenTheme} aria-label="Theme & customization">
            <Palette size={17} />
          </button>
        </div>
        ...
        <div className="calendar-month-title">
          <strong>{MONTH_LABELS[titleMonth] ?? ""}</strong>
        </div>
