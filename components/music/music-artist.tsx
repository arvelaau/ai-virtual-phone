...
export default function MusicArtistPage({ artistId, artistName, onClose }: Props) {
    ...
    const playSong = useCallback(async (song: NeteaseSearchResult) => {
        ...
        if (!info.url) {
            ...
            setToast(info.reason || "This track can't be played right now");
            return;
        }
        const url = info.url;
    ...
    const stats = artist
        ? [
            artist.musicSize ? `${artist.musicSize} songs` : "",
            artist.albumSize ? `${artist.albumSize} albums` : "",
            artist.fansCount ? `${formatFans(artist.fansCount)} followers` : "",
        ].filter(Boolean).join(" · ")
        : "";
    ...
    return (
        <div className="mart-page">
            ...
            <div className="mart-body">
                {loading ? (
                    <div className="mcmt-hint">Loading artist information...</div>
                ) : (
                    <>
                        {artist?.briefDesc && (
                        ...
                        {topSongs.length === 0 && albums.length === 0 && (
                            <div className="mcmt-hint">No works found for this artist</div>
                        )}
                    </>
                )}
            </div>
